...
class _InputPresensiPageState extends State<InputPresensiPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Presensi $status berhasil'),
            backgroundColor: status == 'Hadir' ? Colors.green : Colors.orange,
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) {
          ...
          loadData(); // Refresh untuk update status
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildBody() {
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              'Tambahkan jamaah terlebih dahulu',
    ...
    final tidakHadir = jamaahList
        .where((j) => j.status == 'Tidak Hadir')
        .length;
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            // Tombol aksi (hanya tampil jika sesi aktif dan belum presensi)
